// Package failover provides client-side api-server load balancing for kelper.
//
// kubectl accepts one and only one server per cluster in a kubeconfig. This
// package lets a context's server field hold a comma-delimited list of
// endpoints; Resolve probes each in order and hands downstream tooling a
// temporary single-server kubeconfig pointed at the first reachable endpoint.
package failover
...
// Resolve loads the kubeconfig (from the given path, or the standard discovery
// rules when empty) and resolves the current context's cluster server. When the
// server field holds a comma-delimited list of endpoints it probes each in
// order, logging fallbacks to stdout, and writes a temporary kubeconfig
// pointing at the first reachable endpoint.
//
...
// when every configured endpoint is unreachable. When there is no multi-server
// configuration, the original path is returned unchanged with a no-op cleanup.
func Resolve(kubeconfigPath string) (string, func(), error) {
	noop := func() {}
